    ...
    <header className="sticky top-0 z-40 bg-[#0c0c0e]/95 backdrop-blur-md border-b border-zinc-800/80 px-4 lg:px-8 py-2.5">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {/* CLIENT VIEW: STRICT CLIENT CONTROLS ONLY */}
          {currentRole === 'client' && currentClient && (
            <>
              {/* Authenticated Client Identity Badge */}
              <div className="hidden sm:flex items-center gap-2 bg-zinc-900/90 border border-zinc-800/90 px-3 py-1.5 rounded-xl text-xs">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                <span className="text-zinc-200 font-bold">{currentClient.company}</span>
                <span className="text-zinc-500 text-[11px]">({currentClient.name})</span>
              </div>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="hidden md:flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-zinc-400 hover:text-white text-[11px] font-medium transition-colors"
              ...
              {/* Secure Client Logout Button */}
              <button
                onClick={onLogout}
                className="px-3 py-1.5 rounded-xl bg-zinc-900/90 border border-zinc-800 hover:bg-rose-500/15 hover:border-rose-500/30 hover:text-rose-400 text-zinc-300 text-xs font-bold flex items-center gap-1.5 transition-colors shadow-sm"
                title="تسجيل الخروج من بوابة العميل"
              >
          ...
          {/* ADMIN VIEW: ADMIN CONTROLS ONLY */}
          {currentRole === 'admin' && (
            <>
              {/* Admin Badge */}
              <div className="hidden sm:flex items-center gap-1.5 bg-[#E40107]/10 border border-[#E40107]/25 px-2.5 py-1 rounded-xl text-xs text-[#ff4b4f] font-bold">
                <Shield className="w-3.5 h-3.5" />
                <span>جلسة المشرف العام</span>
              </div>
              ...
              <div className="relative">
                <button
                  ...
                  title="سجل التنبيهات وإشعارات الأمان"
                >
              ...
              <button
                onClick={onLogout}
                className="px-3 py-1.5 rounded-xl bg-zinc-900 border border-zinc-800 hover:bg-rose-500/15 hover:border-rose-500/30 hover:text-rose-400 text-zinc-300 text-xs font-bold flex items-center gap-1.5 transition-colors"
